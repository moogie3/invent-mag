export class SalesReturnCreate {
    ...
    init() {
        if (!this.salesSelect || !this.productReturnList || !this.totalAmountInput || !this.itemsInput) {
            console.error('Required elements not found for SalesReturnCreate');
            return;
        }
        ...
        this.salesSelect.addEventListener('change', this.handleSalesChange.bind(this));
    ...
    handleSalesChange() {
        ...
        if (salesId) {
            fetch(`/admin/sales-returns/sale/${salesId}`)
                .then(response => {
                    if (!response.ok) {
            ...
            this.productReturnList.innerHTML = `
                <div class="empty">
                    ...
                    <p class="empty-subtitle text-muted">Select a sales order to see its items</p>
                </div>
            `;
        }
    }
    ...
    populateItems(items) {
        if (!items || items.length === 0) {
            this.productReturnList.innerHTML = '<div class="text-muted p-4 text-center">No items found in this sales.</div>';
            return;
        }
