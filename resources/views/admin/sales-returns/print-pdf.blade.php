...
<body>
    <div class="header clearfix">
        ...
        <div class="header-right">
            ...
            <div><strong>Ref (Sale): #{{ $salesReturn->sale->invoice }}</strong></div>
            <div>Date: {{ $salesReturn->return_date->format('d F Y') }}</div>
            <br>
            <div class="status-badge">{{ ucfirst($salesReturn->status) }}</div>
        </div>
    </div>
    ...
    <table class="info-table">
        <tr>
            <td style="width: 50%;">
                ...
                {{ $salesReturn->sale->customer->name }}<br>
                {{ $salesReturn->sale->customer->address }}
            </td>
            <td style="width: 50%;">
    ...
    <div class="clearfix">
        <table class="totals-table">
            <tr class="total-row">
                <td>Total Refund:</td>
                <td>{{ \App\Helpers\CurrencyHelper::formatWithPosition($salesReturn->total_amount) }}</td>
            </tr>
        </table>
    </div>
    
    <div style="margin-top: 30px; font-size: 11px; color: #666;">
        Processed by: {{ $salesReturn->user->name ?? 'System' }}
    </div>
