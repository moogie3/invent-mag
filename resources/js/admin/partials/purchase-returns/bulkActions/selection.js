let selectedPurchaseReturnIds = new Set();

export function getSelectedPurchaseReturnIds() {
    return Array.from(selectedPurchaseReturnIds);
}

export function clearPurchaseReturnSelection() {
    const selectAll = document.getElementById("selectAll");
    const checkboxes = document.querySelectorAll(".row-checkbox");
    const bulkBar = document.getElementById("bulkActionsBar");

    selectedPurchaseReturnIds.clear();
...
function setupBulkSelectionListeners({
    ...
    selectAll.addEventListener("change", (e) => {
        rowCheckboxes.forEach((cb) => {
            ...
            if (e.target.checked) {
                selectedPurchaseReturnIds.add(cb.value);
            } else {
                selectedPurchaseReturnIds.delete(cb.value);
            }
        });
...
export function restoreCheckboxStates() {
    document.querySelectorAll(".row-checkbox").forEach((checkbox) => {
        if (selectedPurchaseReturnIds.has(checkbox.value)) {
...
export function updateBulkActionsBarVisibility() {
    ...
    if (bulkActionsBar && selectedCount) {
        const count = selectedPurchaseReturnIds.size;
